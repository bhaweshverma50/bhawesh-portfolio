...
**Pydantic schema it must satisfy** (structured JSON output, so a malformed response is a
...
- SSE with an **automatic poll fallback**: mobile radios kill long connections, so
