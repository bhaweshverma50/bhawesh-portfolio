...
![Drilling into the SpaceLens treemap](/projects/spacelens/explorer-drilldown.gif "root to Downloads to one project, live")

Here's what the first working version looked like, plus the four tricks that got
it production-grade.

## The naive version: one node per file

Version one did the obvious thing: walk the disk, build a `FileNode` per file,
hang children off parents, sum sizes bottom-up. On my machine, with ~4.3 million
...
So the walk now produces a flat dictionary (folder path mapped to total bytes)
and nothing else. No nodes, no parents, no per-file anything:
...
*"N small items"* tile. A folder with 40,000 tiny files becomes ~30 tiles
instead of 40,000.
